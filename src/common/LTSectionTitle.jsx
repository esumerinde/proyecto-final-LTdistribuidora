...
import "./LTSectionTitle.css";

/**
 * Props:
 * - title (string)
 * - subtitle (string opcional)
 * - threshold (0..1) porcentaje visible para disparar. Default 0.35
 * - once (bool) si true, anima solo la 1ra vez. Default true
 * - gradientType (string) "gaming" o "innova" para cambiar el gradient del brush
 */
export default function LTSectionTitle({
  title,
  subtitle,
  threshold = 0.35,
  once = true,
  gradientType = "innova", // Por defecto usa el gradient morado/lavanda
}) {
  // IDs únicos por instancia (evita colisiones)
  ...
  const animRef = useRef(null); // <animate>
  const [played, setPlayed] = useState(false);
  const [showContent, setShowContent] = useState(false); // Estado para mostrar título/subtítulo (final)
  const [progress, setProgress] = useState(0); // Progreso de la animación (0..1)

  // Define el gradient según el tipo
  const gradients = {
    gaming: "linear-gradient(135deg, #f8d56b 0%, #ff8f5a 50%, #ff64d2 100%)",
    innova:
      "linear-gradient(120deg, #7d7cc0ff 0%, #b3b8e6 45%, #d89bfcff 100%)", // Gradient morado personalizado
  };

  const selectedGradient = gradients[gradientType] || gradients.innova;
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (entry.isIntersecting) {
          // Disparar animación SMIL
          if (
            animRef.current &&
            typeof animRef.current.beginElement === "function"
          ) {
            animRef.current.beginElement();
          }
          setPlayed(true);
        } else {
          // Reiniciar estados al salir del viewport
          setPlayed(false);
          setProgress(0);
  ...
  // Sincronizar opacidad del texto con el avance del brush stroke
  useEffect(() => {
    let interval;
    if (played) {
      const duration = 1400; // ms
      const start = performance.now();
      // Cubic bezier: ease-in-out (0.5,0,0.5,1)
      function cubicBezier(t, p0, p1, p2, p3) {
        // Cubic bezier for t in [0,1]
        return (
          Math.pow(1 - t, 3) * p0 +
      ...
      interval = setInterval(() => {
        ...
        // SVG keySplines="0.5,0,0.5,1" → cubic-bezier(0.5,0,0.5,1)
        let eased = cubicBezier(t, 0, 0, 1, 1); // p0=0, p1=0.5, p2=0.5, p3=1
        setProgress(eased);
        if (t >= 1) {
          setShowContent(true);
  ...
  return (
    <>
      <div
        ...
        style={{
          "--clipPathRef": `url(#${ids.clip})`,
          "--brushGradient": selectedGradient,
        }}
      >
        {/* El texto empieza a aparecer cuando el brush lleva un 20% de avance */}
        {(() => {
          const textDelay = 0.2; // 20% de la animación
          const textProgress =
            progress < textDelay ? 0 : (progress - textDelay) / (1 - textDelay);
          const safeProgress = Math.max(0, Math.min(textProgress, 1));
          return (
            <>
              <h2
                className="lt-section-title-main"
                style={{
                  clipPath: `inset(0 ${(1 - safeProgress) * 100}% 0 0)`,
                  WebkitClipPath: `inset(0 ${(1 - safeProgress) * 100}% 0 0)`,
                }}
              >
                {title}
              </h2>
              {subtitle &&
                (typeof subtitle === "string" ? (
                  <p
                    className="lt-section-title-subtitle"
                    style={{
                      clipPath: `inset(0 ${(1 - safeProgress) * 100}% 0 0)`,
                  ...
                  React.cloneElement(subtitle, {
                    ...
                    } lt-section-title-subtitle`.trim(),
                  })
                ))}
            </>
          );
      ...
      {/* Mantener en DOM; no usar display:none */}
      <svg
        width="0"
        height="0"
      ...
      >
        <defs>
          {/* Cortina animada: comienza "indefinite" y la lanzamos por JS al entrar a viewport */}
          <clipPath id={ids.rect}>
